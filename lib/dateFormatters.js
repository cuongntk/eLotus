...
export function getDayNameByDate(dt = 'now') {
  if (!dt) return null
  let arr = ['Chủ nhật', 'Thứ 2', 'Thứ 3', 'Thứ 4', 'Thứ 5', 'Thứ 6', 'Thứ 7']
  if (dt == 'now') return arr[moment().format('d')]
  return arr[moment(dt).format('d')]
}
...
export function dtf2(dt, displayTime = false) {
  ...
  if (moment(dt).format('YYYY') == namnay) {
    ...
    if (moment(dt).format('WW') == parseInt(tuannay) + 1) return moment(dt).format(time + '[' + getDayNameByDate(dt) + ']' + '[ Tuần sau (]DD/MM[)]')
...
export function dtf4(dt) {
  ...
  if (c == 0) {
    ...
    return Math.abs(a.diff(b, 'minutes')) + ' phút'
  }
...
export function dtf5(dt) {
  ...
  if (Math.abs(diffMin) >= 0 && Math.abs(diffMin) < 60) return dtf5Prefix(diffMin) + Math.abs(diffMin) + ' phút'

  if (Math.abs(diffHour) >= 1 && Math.abs(diffHour) < 24)
    return dtf5Prefix(diffHour) + Math.abs(diffHour) + ' giờ ' + Math.abs(diffMin - diffHour * 60) + ' phút'
...
export function dtf6(dt) {
  ...
  if (Math.abs(diffSec) >= 0 && Math.abs(diffSec) < 60) return Math.abs(diffSec) + ' giây' + dtf6Suffix(diffSec)

  if (Math.abs(diffSec) >= 60 && Math.abs(diffSec) < 60 * 60) return Math.abs(diffMin) + ' phút' + dtf6Suffix(diffSec)

  if (Math.abs(diffHour) >= 1 && Math.abs(diffHour) < 24) return Math.abs(diffHour) + ' giờ' + dtf6Suffix(diffHour)

  if (Math.abs(diffDay) >= 1 && Math.abs(diffDay) < 30) return Math.abs(diffDay) + ' ngày' + dtf6Suffix(diffDay)
...
export function dtf7(dt) {
  ...
  if (timeDetail.hours === 0) return `${timeDetail.minutes} phút trước ${moment(dt).format(`[(lúc ]H[h]mm['] [hôm nay)]`)}`

  if (diffDay === 0) return `${timeDetail.hours} giờ trước ${moment(dt).format(`[(lúc ]H[h]mm['] [hôm nay)]`)}`
